...
* In the File Transfer dialog, the local and remote sides support a Places feature where you can define places (e.g. "Log Folder"), backed by one or more folder locations. Selecting a place moves you to the first folder location that exists at the remote. This lets you have easy access to places that are deeply nested, or that have different physical locations on different groups of fielded systems.
